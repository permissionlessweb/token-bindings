...
pub fn execute_change_owner(
    ...
    CONFIG.update(deps.storage, |mut state| {
        ...
        state.owner = match new_owner.len() > 2 {
            true => api.addr_validate(&new_owner)?,
            false => return Err(ReflectError::Std(StdError::msg("human address too short"))),
        };
        Ok(state)
    })?;
...
pub fn reply(
    ...
    RESULT.save(deps.storage, &key, &msg)?;
    Ok(Response::default())
}
...
mod tests {
    ...
    use cosmwasm_std::{
        coin, coins, from_json, BalanceResponse, BankMsg, BankQuery, Binary, Coin, Event,
        StakingMsg, SubMsgResponse,
    };
    ...
    fn proper_instantialization() {
        ...
        assert_eq!(creator.as_str(), value.owner.as_str());
    }
    ...
    fn reflect_requires_owner() {
        ...
        let info = message_info(&deps.api.addr_make("random"), &[]);
    ...
    fn change_owner_works() {
        ...
        assert_eq!(0, res.messages.len());
        let value = query_owner(deps.as_ref()).unwrap();
        assert_eq!(new_owner.as_str(), value.owner.as_str());
    }
    ...
    fn change_owner_errors_for_invalid_new_address() {
        ...
        match err {
            ReflectError::Std(e) => {
                assert!(e.to_string().contains("human address too short"))
            }
